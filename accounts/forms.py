...
class EditProfileForm(forms.ModelForm):
    first_name = forms.CharField(widget=forms.TextInput(attrs={'class': 'shadow-sm focus:ring-indigo-500 dark:bg-dark-third dark:text-dark-txt focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md',}), required=False)
    ...
    pinture = forms.ImageField(label='Profile Picture',required=False, widget=forms.FileInput)
    ...
    class Meta:
        model=Profile
        fields = ('first_name','last_name','pinture','banner','location','url','bio','birthday',)